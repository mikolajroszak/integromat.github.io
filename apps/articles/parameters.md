...
| **editable** | boolean | If `true`, the user can manually edit the value of this parameter (or use mappings). Default: `false`.                             |
| **mode**     | string  | Specifies the initial editing mode when **editable** set to `true`. If `"edit"`, the field will be initially set to **mapping mode**. If not present, the field will be initially set to **select mode**. |
